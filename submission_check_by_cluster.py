...
result_reversefit = np.loadtxt('submission/baseline_'+str(baseline)+'_clus_'+str(cluster)+'_predict.csv',delimiter=',')

# visualizing check
counter = 0
for i in cluster_file:
    print counter
    data = pd.read_csv('flow_per_shop/' + str(i) + '.csv',delimiter=',')
