...
def save_label(buffer_csv,row_num,count):
    ...
    user_times = info.groupby(by='user_id',sort=False)['shop_id'].count()
    user_times = pd.DataFrame(user_times)

    user_fluent = user_times.index.values[user_times.values.reshape(-1)>5]
    user_rere = user_times.index.values[user_times.values.reshape(-1)<=5]

    info_fluent = info.loc[info['user_id'].isin(user_fluent)]
    info_rare = info.loc[info['user_id'].isin(user_rere)]

    # print info_fluent
    # print info_rare

    target_fluent = build_target()
    info_fluent = pd.merge(target_fluent,info_fluent,how='left',on=['year','month','day'])
    target_fluent['count'] = info_fluent.groupby(by=['year','month','day'])['user_id'].count().values
    # print target_fluent

    target_rare = build_target()
    info_rare = pd.merge(target_rare,info_rare,how='left',on=['year','month','day'])
    target_rare['count'] = info_rare.groupby(by=['year','month','day'])['user_id'].count().values
    # print target_rare

    target_fluent.to_csv('flow_per_shop/'+str(row_num)+'_fluent.csv')
    target_rare.to_csv('flow_per_shop/'+str(row_num)+'_rare.csv')
